...
get_network_DAG <- function(net){
  ...
  true_dag <- amat(network)
  
  network_cpdag <- cpdag(network)
  true_cpdag <- amat(network_cpdag)
...
check_sims_created <- function(n){
  ...
  if (dir.exists(sim_file)){
    files <- list.files(sim_file)
    files_of_interest <- c("data[[:digit:]]+.txt")
    sims_not_created <- !any(str_detect(files,files_of_interest))
  } else {
